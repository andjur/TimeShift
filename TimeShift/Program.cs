    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var file in files)
            {
                ...
                match = Regex.Match(file, @".*\\((.*)(\d{4}).(\d{2}).(\d{2}).(\d{2}).(\d{2}).(\d{2})(.*))", RegexOptions.IgnoreCase);
                if (match.Success)
                {
                    var dt = new DateTime(
                        int.Parse(match.Groups[3].Value), int.Parse(match.Groups[4].Value), int.Parse(match.Groups[5].Value),
                        int.Parse(match.Groups[6].Value), int.Parse(match.Groups[7].Value), int.Parse(match.Groups[8].Value));
                    dt = dt.AddHours(signum * shiftHours);
                    dt = dt.AddMinutes(signum * shiftMinutes);
                    dt = dt.AddSeconds(signum * shiftSeconds);
                    Console.WriteLine("ren \"{0}\" \"{1}\"", match.Groups[1].Value, match.Groups[2].Value + dt.ToString("yyyy-MM-dd_HH.mm.ss") + match.Groups[9].Value);
                }
            }
